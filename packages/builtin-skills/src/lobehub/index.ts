...
export const LobeHubSkill: BuiltinSkill = {
  ...
  description:
    "Manage the 小逆chat platform via the `lh` CLI — INCLUDING modifying THIS agent's own configuration. ACTIVATE this skill whenever the user asks you to: change your system prompt / instructions / persona, enable or disable tools / plugins / skills, switch model or provider, attach knowledge bases or files, edit the opening message, rename the topic, OR operate on platform resources (agents, topics, memory, documents, search, model/provider/plugin management, bot integrations, generation task administration, evals, usage stats). ALSO ACTIVATE when the user asks to connect, link, or set up a messaging platform bot — including Discord, Telegram, Slack, Feishu (飞书), Lark, QQ, or WeChat (微信) — or uses phrases like '帮我链接 Discord', 'connect my Slack', '接入飞书', '配置 QQ 机器人', 'link WeChat'. DO NOT activate this skill for an ordinary request to generate or edit a raster image; use the native image-generation tool instead. Only use the generation CLI when the user explicitly asks to operate `lh gen` or administer an existing generation task. Without activation you cannot persist any platform change — you can only describe what you would do.",
