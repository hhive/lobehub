 ...
 * by showing 小逆chat title instead of the agent's own meta.
...
export const useAgentMeta = (messageAgentId?: string | null): MetaData => {
  ...
  return useMemo(() => {
    ...
    if (isBuiltinAgent) {
      // Use DB-stored title if customized (e.g. via onboarding), otherwise fallback to 小逆chat
