...
export interface AgentSelfIterationCapabilityInput {
  ...
  /**
   * Whether the agent is the product-owned 小逆chat agent.
   */
  isLobeAiAgent: boolean;
}

/**
 * Checks whether a slug belongs to 小逆chat.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as 小逆chat
 *
 * Expects:
 ...
 * - 小逆chat should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for 小逆chat when the feature flag is enabled
 * - `true` for non-小逆chat only when `agentSelfIterationEnabled` is true
 */
